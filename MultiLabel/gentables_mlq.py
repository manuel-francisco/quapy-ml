...
def generate_table(path, protocol, error, include, color=True):#, drift_bin):
    ...
    def compute_score_job(dataset, model):
        if include == "all" or any(model.endswith(e) for e in include):
            ...
            if os.path.exists(result_path):
                ...
                true_prevs, estim_prevs, train_prevs = result[protocol]
                train_prevs = np.asarray(train_prevs)
                scores = np.asarray([error(trues, estims) for trues, estims in zip(true_prevs, estim_prevs)])

                drifts = np.asarray([error(trues, train_prevs) for trues, _ in zip(true_prevs, estim_prevs)])
                drifts = np.mean(drifts, axis=1)
                min_drifts = np.min(drifts)
                delta_drifts = (np.max(drifts) - min_drifts) / 3

                for drift_bin in range(3):
                    lower_bound = min_drifts + drift_bin * delta_drifts
                    upper_bound = min_drifts + (drift_bin + 1) * delta_drifts
                    scores[np.isnan(scores).any(axis=1), :] = 999999
                    s_aux = scores[(drifts >= lower_bound) & (drifts < upper_bound), :].flatten()
    ...
    table = MultiMethodTable(sorted(datasets), filtered_models, prec_mean=4, clean_zero=True, significance_test=None, color=color) # FIXME significance_test='wilcoxon'
